...
I<Autrijus Tang (L{<http://www.autrijus.org/>)" is a talented Perl hacker,
...
B<Autrijus:> Ok. The story is that I hacked SVK (L<http://svk.elixus.org/>)
for many months with clkao (L<http://search.cpan.org/%7Eclkao/>). SVK worked,
except it is not very flexible. There is a VCS named darcs
(L<http://abridgegame.org/darcs/>), which is much more flexible,
but is specced using quantum physics language and written in a scary language
called Haskell (L<http://www.haskell.org/>). So, I spent one month doing
nothing but learning Haskell, so I could understand darcs. Which worked well;
I convinced  a crazy client (who paid me to develop I<Parse::AFP>
(L<http://search.cpan.org/perldoc?Parse::AFP"></a>) that Perl 5 is doomed
because it has no COW (which, surprisingly, it now has), and to fund me to
...
B<chromatic:>  You should see I<Attributes::Scary>.  Total sympathy here.

B<Autrijus:> I<Template::Extract> 
(L<http://search.cpan.org/perldoc?Template::Extract>) uses C<(?{})> as a 
nondet engine; PAR (L<http://search.cpan.org/perldoc?PAR>) comes with its 
own I<perlmain.c>; let me not mention source filtering. All these techniques
are unmaintainable unless with large dosage of caffeine.

B<chromatic:>  Yeah, I fixed some of the startup warnings in I<B::Generate>
(L<http://search.cpan.org/perldoc?B::Generate>) a couple of weeks ago...

B<Autrijus:> Cool. Yeah, B::Generate is abstracted klugery
...
B<Autrijus:> Actually, I think Pugs and Parrot will meet in the middle.
Where Pugs AST meets Parrot AST and the compiler is written in Perl 6
that can then be run on Parrot.

B<chromatic:> I thought Pugs would get rewritten in C for
Parrot?

B<Autrijus:> No, in Perl 6.

B<chromatic:>  Can GHC (L<http://www.haskell.org/ghc/>) 
retarget a different AST then?

B<Autrijus:> It can, but that's not the easier plan.

B<chromatic:>  It's easy for me.  I don't plan to do it.

B<Autrijus:> The easier plan is simply for Pugs to have a I<Compile.hs> that
...
B<Autrijus:> Parrot is fine except every time I build it, it
fails.

B<chromatic:>  Try running Linux PPC sometime.

B<Autrijus:> Freebsd may not be a good platform for Parrot, I gathered.
...
B<Autrijus:> The purely functional Perl 6 plan stops at day 3.
...
B<chromatic:> Does Haskell support continuations and all of those funky
things?
...
B<Autrijus:> Each comes with its own penalty, but is
generally small.  GHC, again, compiles to very fast C code.

B<chromatic:> Can you instrument scopes at runtime too?

B<Autrijus:> Sure.  <code>&?CALLER::SUB</code> works.  And
<code>$OUTER::var</code>.

B<chromatic:> Are you compiling it to native code now?
I remember that being a suggestion a few days ago.

B<Autrijus:> Pugs itself is compiled to native code; it is
...
B<Autrijus:> Cool.  So yeah, it's like Perl 5 now.  The
...
B<Autrijus:> They don't exist.  6.2.x will do that, though.
...
B<Autrijus:> Yup, thanks to #Perl6.  I seldom write tests now :)
...
B<Autrijus:> It indeed is. I get svn/svk patches and darcs
patches.

B<chromatic:> Is there a lot of overlapping interest?  Where does it come
from?

B<Autrijus:> Well, ever since the monadic revolution of '98
...
B<Autrijus:> Yeah, Ingy is working on it I<ext/Kwid/> eventually inline
...
of the I<examples/*> started that way.
