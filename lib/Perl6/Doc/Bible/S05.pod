...
A C<Â«> or C<<< << >>> token indicates a left word boundary.  A C<Â»> or
...
standard hash access notations (C<$/{'foo'}>, C<< $/<bar> >>, C<$/Â«bazÂ»>,
etc.), or else via corresponding lexically scoped aliases (C<< $<foo> >>,
C<$Â«barÂ»>, C<< $<baz> >>, etc.)  So the previous example also implies:
